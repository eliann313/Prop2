...
export default async function PaginaFavoritos() {
  ...
  const ids = favoritos.map(({ publicacion }) => publicacion.id);

  return (
    // Acá el estado se conoce sin preguntar —todo lo que se lista ES favorito— así que se
    // siembra y el provider no hace ningún pedido. La página es privada y nunca se cachea.
    <FavoritosProvider idsEnPagina={ids} favoritosIniciales={ids}>
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {favoritos.map(({ publicacion }) => (
            <TarjetaDePublicacion
              ...
              accion={
                <BotonFavorito publicacionId={publicacion.id} volverA={RUTAS.favoritos} />
              }
            />
          ))}
        </div>
